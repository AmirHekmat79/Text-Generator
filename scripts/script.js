let $ = document;
let numberInputElem = $.querySelector('#numberInput');
let GenerateBtn = $.querySelector('.btn');
let ParagraphsElem = $.querySelector('.Paragraphs');
const dummyText = [
...
GenerateBtn.addEventListener('click',function(){
    ParagraphsElem.innerHTML = '';
    let paragraphCount = numberInputElem.value;
    if(paragraphCount != '' && !(paragraphCount < 0) && !(paragraphCount > 9) && !(isNaN(paragraphCount))){
         for(let i=0 ; i < paragraphCount ; i++){
          let divElem = $.createElement('div');
          divElem.innerHTML = dummyText[Math.floor(Math.random() * dummyText.length)];
          divElem.classList.add('my-3');
          ParagraphsElem.append(divElem);
      }  
    }else{
      let divElem = $.createElement('div');
      divElem.innerHTML = dummyText[Math.floor(Math.random() * dummyText.length)];
